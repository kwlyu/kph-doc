...
- Arrival: You're typically scheduled to arrive **1-2 hours before** the event. Performers typically arrive 15-30 minutes after you arrive.
    ...
    - Set Outputs 1 & 3 (Crestron & Preview) to QLab. (You can also achieve this via `Macro 2` in ATEM.)
...
- Preshow:
    ...
    - When done, bring the QLab input (`Input 7`) to `Program`, and set Outputs 1 & 3 (Crestron & Preview) to `Program`. (You can also achieve this via `Macro 1` in ATEM.)
    - We typically start and end transitions with a wide shot; you can use `House Center` for this. When you're done, bring `Input 2` to `Preview`.
