...
{% include image.html file="basic-title.png" alt="Basic Title" caption="Basic Title with no Performers"%}

For instance, `Cue 1` contains a total of 5 cues, including itself. This cue was created for organizational purposes, since it doesn't affect the playback behavior. (That is, playback will behave the same as if one plays the first cue inside the group.) It is indicated by a purple border around the cues that it contains. I use group cues to indicate the *scope* of a group of lower thirds.

{% include image.html file="group-mode-1.png" alt="Group Mode" caption="Start first and enter mode"%}

The next cue after `Cue 1` is also a group cue. This time, it is in **Timeline** mode; all children of this cue will begin playback simultaneously. Note that one can achieve the same result by setting everything to `Auto Continue`. Doing it this way with a group cue gives me a clear indication of the scope of all cues being played back.

{% include image.html file="group-mode-2.png" alt="Group Mode" caption="Timeline mode"%}
